...
// app/account/cart/page.tsx  (or wherever your Cart component lives)
//
// ─── WHAT CHANGED AND WHY ────────────────────────────────────────────────────
// REMOVED: The add_to_cart useEffect that was firing on page load.
//          That was semantically wrong — visiting the cart page is passive
//          viewing, not an "add to cart" user action. It was inflating GA4
//          add_to_cart numbers. The correct place for that event is in
//          EngineProductClient when the user actually clicks the button.
//
// REMOVED: const addToCartFiredRef = useRef(false) — no longer needed.
//
// REMOVED: const { items } = useCartStore() — this was a duplicate reference.
//          cartItems from useCartStore((s) => s.items) already holds the same
//          data. items.length in the JSX is replaced with cartItems.length.
//
// ADDED:   import { trackRemoveFromCart } from "@/lib/gtm"
// ADDED:   handleRemoveFromCart wrapper function that tracks BEFORE removing.
//          We track first while we still have the item data (price, name),
//          then call removeItem. If we called removeItem first, the data
//          would be gone and we couldn't send it to the data layer.
//
// ALL existing functionality — layout, checkout navigation, quantity updates,
// coupon input, error handling, cart display — is completely unchanged.
...
import Link from "next/link";
import { useCartStore } from "@/store/cartStore";
import type { CartItem } from "@/store/cartStore"; // ← single source of truth, no local interface
import { getProductCategory, trackRemoveFromCart } from "@/lib/gtm";
...
export default function Cart() {
  ...
  // Track BEFORE removing — once removeItem runs the item data is gone
              ...
              cartItems.map((item) => (
                ...
                >
                  ...
                  <div className="flex flex-col md:flex-row md:col-span-6 justify-between items-center w-full">
                    ...
                    <div className="flex justify-between w-full md:w-auto md:col-span-2 md:justify-center mb-2 md:mb-0">
                      <div className="flex items-center space-x-1 text-white rounded-md border border-gray-400 bg-gray-800 px-2">
                        ...
                        <input
                          ...
                          value={item.quantity}
                          onChange={(e) =>
                            updateQuantity(item.id.toString(), parseInt(e.target.value) || 1)
                          }
                          className="w-8 text-center bg-transparent text-lg text-white focus:outline-none"
                          min="1"
                        />
                    ...
                    <div className="relative flex justify-between md:justify-end items-start space-x-2 font-exo2 font-bold text-white text-lg w-full md:w-auto md:col-span-2">
                      <span className="md:hidden">Subtotal:</span>
                      <span className="flex-1 md:flex-initial text-right md:text-left">
