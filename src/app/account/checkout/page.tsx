...
// REMOVED: cartItemsArray variable — it was built for the old manual
//          window.dataLayer.push that we replaced. Now dead code, deleted.
//
// REMOVED: The old begin_checkout useEffect that used (window as any).__beginCheckoutFired
//          as a guard. That pattern is fragile — the window flag survives
//          across re-renders inconsistently. Replaced with useRef which is
//          the React-correct way to track "has this fired" state.
//
// ADDED:   import { trackBeginCheckout } from "@/lib/gtm"
// ADDED:   beginCheckoutFired useRef guard
// ADDED:   Clean begin_checkout useEffect using trackBeginCheckout()
//
// ALL existing functionality — OTP verification, form validation, Redis
// checkout saving, billing/shipping forms, terms acceptance, navigation —
...
import { getProductCategory, trackBeginCheckout } from "@/lib/gtm"; // ← NEW: import from centralized utility
...
export default function Checkout() {
  ...
  // ─── NEW: Guard to prevent begin_checkout from firing more than once ──────
  // useRef is the React-correct way to track "did this already run" state.
  // Unlike useState, changing a ref does not cause a re-render.
  // Unlike window flags, refs are scoped to this component instance and
  // are cleaned up properly when the component unmounts.
  ...
  useEffect(() => {
    const loadSavedData = async () => {
      try {
        ...
        if (savedData) {
          if (savedData.shippingInfo) {
            setFormData((prev) => ({ ...prev, ...savedData.shippingInfo }));
            setShippingInfo(savedData.shippingInfo);
          }
          if (savedData.billingInfo && savedData.billingInfo !== null) {
            const billing = savedData.billingInfo;
  ...
  useEffect(() => {
    if (shippingInfo) {
      setFormData((prev) => ({ ...prev, ...shippingInfo }));
    }
  }, [shippingInfo]);
  ...
  useEffect(() => {
    ...
    formData.company, formData.country, formData.address,
    formData.apartment, formData.city, formData.state,
    formData.zipCode, userType,
  ]);
  ...
  const formatPhoneNumber = (value: string, country: "US" | "IN" = "US") => {
    ...
    if (country === "US") {
      ...
      else return `${limited.slice(0, 5)}-${limited.slice(5)}`;
    }
    return cleaned;
  };
  ...
  const handleContinue = async () => {
    ...
    const saveSuccess = await saveCheckoutData(
      {
        ...
        shippingInfo: { ...formData, userType },
        billingInfo: { ...billingFormData, userType: billingUserType, sameAsShipping },
        verification: { isVerified, phoneNumber: formData.phone },
        cartItems: items.map((item) => ({
          id: item.id,
          name: item.name,
          title: item.title,
        ...
        orderTotal: { subtotal, shipping: orderShipping, taxes: orderTaxes, total: orderTotal },
        termsAccepted,
        isOrderCreatedInBackend: false,
      },
      sessionId
    ...
    if (saveSuccess) {
      console.log("✅ Checkout data with cart items saved to Redis successfully");
    }
  };
  ...
  const isFormValid = () => {
    ...
    const billingValid =
      sameAsShipping ||
      (formData.firstName && formData.lastName && formData.phone &&
        billingFormData.country && billingFormData.address &&
        billingFormData.city && billingFormData.state && billingFormData.zipCode);
  ...
  // ─── NEW: begin_checkout tracking ────────────────────────────────────────────
  // Fires once when the checkout page first mounts with items in the cart.
  //
  // WHY useRef instead of a window flag:
  // useRef persists across re-renders without causing re-renders itself.
  // Setting beginCheckoutFired.current = true prevents double-firing if React
  // re-renders this component (e.g. due to a state update elsewhere on the page).
  //
  // WHY empty dependency array []:
  // We want this to fire exactly once when the page mounts — not every time
  // the cart changes. The user arriving at checkout is a single event.
  //
  // REMOVED: cartItemsArray variable that was here for the old dataLayer push.
  useEffect(() => {
    if (typeof window === "undefined") return; // don't run on server
    if (!cartItems.length) return;             // don't fire with empty cart
    if (beginCheckoutFired.current) return;    // don't fire more than once
    ...
    trackBeginCheckout(
      cartItems.map((item) => ({
        ...
        item_name: item.title || item.name,
        item_category: getProductCategory(item),
        price: item.price,
        quantity: item.quantity,
      })),
      estimatedTotal
    ...
    <div className="min-h-screen bg-[#091B33] text-[#FFFFFF] pt-16 md:pt-14 pb-22 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-6 py-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="flex justify-end items-end text-right mt-2 pr-1 md:pr-6 mb-4">
              <Link href="/account/sign-in" className="text-sm text-[#FFFFFF] hover:underline font-exo2">
                Have an account? Log in
              </Link>
            </div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 items-center">
                      <input
                        ...
                        onChange={(e) => {
                          let value = e.target.value;
                          if (value.startsWith("+1")) value = value.slice(2);
                          handleInputChange("phone", value);
                        }}
                        className="w-full bg-[#1A263D] border border-[#484848] text-[#FFFFFF] rounded-md px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#009AFF] font-exo2 transition-all duration-200"
                        required
                        disabled={otpSent && !isVerified}
                      />
                      {!otpSent && (
                        <button
                          type="button"
                          onClick={sendOtp}
                          className="bg-[#009AFF] hover:bg-[#00B8FF] active:scale-95 transition-all duration-200 px-4 py-0.5 rounded-md text-white font-exo2 shadow-md hover:shadow-[#009AFF]/50"
                        >
                          Send Code
                        </button>
                      )}
                    ...
                    {otpSent && !isVerified && (
                      <div className="flex gap-2 items-center mt-2">
                        ...
                        <button
                          type="button"
                          onClick={verifyOtp}
                          className="bg-green-500 hover:bg-green-600 active:scale-95 transition-all duration-200 px-4 py-2 rounded-md text-white font-exo2 shadow-md hover:shadow-green-400/40"
                          disabled={isVerified}
                        >
                          Verify
                        </button>
                    ...
                    {isVerified && <p className="mt-2 text-green-400 font-exo2 text-sm"></p>}
                  </div>
                </div>
              </form>
            </div>
            ...
            <div className="mb-8">
              ...
              <div className="flex items-center mb-4">
                <input
                  ...
                  className="form-checkbox h-4 w-4 text-[#009AFF] bg-transparent border-white rounded mt-1 focus:ring-1 focus:ring-offset-0 focus:ring-white"
                />
                <label htmlFor="sameAsShipping" className="ml-2 text-sm md:text-base text-[#FFFFFF] font-exo2 leading-relaxed">
                  My billing address is the same as the shipping address
                </label>
              </div>
            ...
            <div className="mt-8">
              <div className="flex items-start mb-6">
                ...
                <label htmlFor="terms" className="flex items-start">
                  <span className="text-sm md:text-lg font-exo2">
                    I have Read and Accept the
                    <Link href="/account/terms-and-condition" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline"> terms and conditions</Link>,
                    <Link href="/account/warranty-and-return" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline"> return policy</Link>
                    {""} and the{" "}
                    <Link href="/account/privacy-policy" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Privacy Policy</Link>{" "}
                    of partscentral.us *
                  </span>
                </label>
              </div>
          ...
          <div className="lg:col-span-1 h-fit md:mt-10 w-full">
            <div className="bg-[#02305A] rounded-lg p-4 sm:p-6 mb-6">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-2">
                ...
                <Link href="/account/cart" className="text-[#009AFF] hover:underline font-exo2 text-sm text-center sm:text-right">
                  Edit cart
                </Link>
              </div>
                  ...
                  >
                    <div className="flex items-start space-x-3 flex-grow">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-exo2 text-sm sm:text-base text-white font-medium break-words">{item.subtitle}</p>
                        <p className="font-exo2 text-xs sm:text-sm text-gray-300 mt-1">{item.quantity} pc.</p>
                      </div>
                    </div>
                    <div className="flex-shrink-0 ml-auto max-w-[100px] sm:max-w-[120px] text-right">
                      <p className="font-exo2 text-base sm:text-lg md:text-xl text-white font-bold truncate break-words">
            ...
            <div className="bg-[#02305A] p-4 sm:p-6 rounded-lg shadow-lg sticky top-24 max-h-fit">
              ...
              <div className="mt-6 pt-4 md:px-4 flex justify-between items-center">
                <span className="text-white text-base sm:text-lg font-bold font-exo2">Estimated Total:</span>
                <span className="text-white text-lg sm:text-xl font-bold font-exo2">${estimatedTotal.toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
