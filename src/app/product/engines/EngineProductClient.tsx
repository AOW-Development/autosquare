...
// app/engines/EngineProductClient.tsx
//
// ─── WHAT CHANGED AND WHY ────────────────────────────────────────────────────
// ADDED:   import { trackProductView, trackAddToCart } from "@/lib/gtm"
//
// REPLACED: The old product_view useEffect that had two problems:
//           1. It pushed event: "product_view" but GTM listens for "view_item"
//              (GA4 standard name). Those strings must match exactly.
//           2. It had [selectedProduct, selectedSubPartId, groupedVariants] as
//              dependencies, firing on every dropdown change — a user switching
//              mileage options shouldn't count as 10 separate product views.
//           NEW version uses a useRef guard so it fires ONCE per page load,
//           only after a real product SKU is available, and uses "view_item".
//
// ADDED:   trackAddToCart() inside handleAddToCartAndRedirect — this is the
//          correct semantic moment because the user has confirmed their vehicle
//          fitment in the VerifyPartPopup and committed to adding the item.
//          We do NOT track in handleAddToCart because that just opens the popup.
//
// ADDED:   trackAddToCart() inside handleBuyNow — this was completely untracked
//          before. Without it, buy-now purchases showed up in GA4 as purchases
//          with no add_to_cart event, breaking funnel analysis.
//
// ALL existing functionality — variant selection, dropdowns, media loading,
// URL updates, popup handling, VerifyPartPopup, accordion tabs, buy now,
// part request, out-of-stock handling — is completely unchanged.
...
import { useRouter } from "next/navigation";
import { VerifyPartPopup } from "@/components/fitment";
import { trackProductView, trackAddToCart } from "@/lib/gtm"; // ← NEW: tracking imports
...
const getDisplayPrice = (product: any): { price: number; showDiscount: boolean; originalPrice: number | null } => {
  ...
  if (discountedPrice > 0 && discountedPrice < actualPrice) {
    return { price: discountedPrice, showDiscount: true, originalPrice: actualPrice };
  }

  return { price: actualPrice, showDiscount: false, originalPrice: null };
};
...
export default function EngineProductClient({
  ...
  // ─── NEW: Guard for product view tracking ─────────────────────────────────
  // useRef persists across renders without causing re-renders.
  // We set this to true after the first fire so it never fires again,
  // even if React re-renders the component (e.g. dropdown changes).
  const productViewFired = useRef(false);
  ...
  useEffect(() => {
    ...
    if (initialProduct) {
      setSelectedProduct(initialProduct);
      setSelectedSubPartId(
        initialProduct.subPart?.id || initialData.groupedVariants[0]?.subPart?.id || null,
      );
      setSelectedMilesSku(
        initialProduct.sku || initialData.groupedVariants[0]?.variants?.[0]?.sku || "",
      );
    }
  ...
  // ─── UPDATED: handleBuyNow with add_to_cart tracking ─────────────────────
  // ADDED: trackAddToCart() call after addItem().
  // WHY: Buy-now bypasses handleAddToCartAndRedirect entirely, so without
  // this call, buy-now purchases showed up in GA4 as purchases with zero
  // corresponding add_to_cart events — breaking funnel analysis entirely.
  const handleBuyNow = () => {
    if (!selectedProduct) return;
    ...
    addItem({
      ...
      subtitle:
        (selectedProduct.specification || "") + " " + (selectedProduct.miles || "") || selectedProduct.sku,
      image:
    ...
    // ✅ ADDED: Track add_to_cart for the buy-now flow.
    // We track AFTER addItem so the cart store is already updated.
    trackAddToCart(
      {
        item_id: selectedProduct.sku,
        item_name:
          selectedProduct.title ||
          `${selectedProduct.year || ""} ${selectedProduct.make || ""} ${selectedProduct.model || ""} ${selectedProduct.part || ""}`.trim(),
        item_category: selectedProduct.part || "",
        price,
        quantity,
      },
      ...
      content: (
        <>
          <p>
            This {productInfo.make} {productInfo.model} {productInfo.part} is{" "}
            {productInfo.year ? `from ${productInfo.year} ` : ""}models. Each{" "}
            {productInfo.part} is tested and ready to install and offers improved performance.
          </p>
          <br />
          <p>
            This Unit is perfect for anyone in the market for reliable used{" "}
            {productInfo.part} that will offer superior results - a great addition to any repair project!
          </p>
          <br /><br />
        </>
      ),
      ...
      content: (
        <>
          <div className="flex space-x-[8px]">
            ...
            <Image src="/Images/home/americanexpress_82060.png" alt="Amex" width={48} height={30} className="bg-[#1E2A44] object-contain" />
  ...
  // ─── UPDATED: Product view tracking ──────────────────────────────────────
  // REPLACED: The old useEffect that pushed event: "product_view" with
  //           dependencies [selectedProduct, selectedSubPartId, groupedVariants],
  //           which fired on every dropdown change.
  //
  // NEW version:
  // - Uses "view_item" (GA4 standard, matches GTM trigger exactly)
  // - Uses productViewFired ref so it fires ONCE per page load only
  // - Only fires after a real product SKU is available
  // - Dependency is [selectedProduct?.sku] — only re-fires if user somehow
  //   ends up on a completely different product (which doesn't happen here
  //   but is a safe fallback)
  useEffect(() => {
    if (!selectedProduct?.sku || productViewFired.current) return;

    productViewFired.current = true;
  ...
  const handleAddToCart = () => {
    ...
    setSelectedProductForVerify({ ...selectedProduct, subPart: selectedGroup.subPart });
    setIsVerifyPopupOpen(true);
  };

  // ─── UPDATED: handleAddToCartAndRedirect with add_to_cart tracking ───────
  // ADDED: trackAddToCart() call after addItem().
  // WHY: This is the correct semantic moment for the add_to_cart event.
  // The user has been through the VerifyPartPopup and confirmed their vehicle
  // fitment. They've committed to this product. We track AFTER addItem so
  // the cart store is already updated when the event fires.
  const handleAddToCartAndRedirect = (product: any, quantity = 1) => {
    const { price } = getDisplayPrice(product);

    // Existing addItem call — unchanged
    addItem({
      id: product.sku,
    ...
    // ✅ ADDED: Track add_to_cart after item is in cart store.
    // GTM picks this up and fires "GA4 Add to Cart" tag.
    trackAddToCart(
      {
        item_id: product.sku,
        item_name:
          product.title ||
          `${product.year || ""} ${product.make || ""} ${product.model || ""} ${product.part || ""}`.trim(),
        item_category: product.part || "",
        price,
        quantity,
      },
      price * quantity
            ...
            <div className="flex flex-col md:flex-row gap-1 md:gap-8 items-start pt-0 md lg:pt-10">
              <div className="flex justify-center lg:justify-start pt-4 md:pt-8">
                <div className="relative w-[250px] h-[250px] sm:w-[300px] sm:h-[300px] md:w-[350px] md:h-[350px] lg:w-[400px] lg:h-[400px] bg-[#12263A] rounded-lg flex flex-col items-center justify-center">
                  {mediaUrls.length > 0 ? (
                    <>
                      <a href={mediaUrls[0]} target="_blank" rel="noopener noreferrer" className="cursor-pointer" style={{ display: "contents" }}>
                        <Image
                          src={mediaUrls[0]}
                          alt={`${selectedProduct?.title || part} image`}
                          width={250}
                          height={160}
                          className={`object-contain py-4 px-8 md:p-4 md:w-80 md:h-300 lg:w-100 lg:h-350 ${imageLoaded ? "opacity-100" : "opacity-0"} transition-opacity duration-200`}
                          priority
                          onLoadingComplete={() => setImageLoaded(true)}
                        />
                      </a>
                    ...
                    <>
                      <a href="https://s3.us-east-1.amazonaws.com/partscentral.us/public/engine-1.png" target="_blank" rel="noopener noreferrer" className="cursor-pointer" style={{ display: "contents" }}>
                        <Image
                          src="https://s3.us-east-1.amazonaws.com/partscentral.us/public/engine-1.png"
                          alt="Engine"
                          width={250}
                    ...
                    <>
                      <a href="https://s3.us-east-1.amazonaws.com/partscentral.us/Trasmission_.png" target="_blank" rel="noopener noreferrer" className="cursor-pointer" style={{ display: "contents" }}>
                        <Image
                          src="https://s3.us-east-1.amazonaws.com/partscentral.us/Trasmission_.png"
                          alt="Transmission"
                          width={250}
              ...
              <div className="flex flex-col gap-4 pt-4 md:pt-8 lg:pl-1 w-full lg:w-[80%]">
                ...
                >
                  {(() => {
                    const year = selectedProduct?.year || productInfo.year || "";
                    const make = selectedProduct?.make || productInfo.make || "";
                    const model = selectedProduct?.model || productInfo.model || "";
                    const part = (selectedProduct?.part || productInfo.part || "").toLowerCase();
                    const partName = part.includes("engine") ? "Engine" : "Transmission";
                    return `${year} ${make} ${model} Used ${partName}`.trim();
                  })()}
                </h1>
                ...
                <div className="text-sm sm:text-base text-gray-400 font-semibold">
                  options:{" "}
                  {groupedVariants.find((g) => g.subPart.id === selectedSubPartId)?.subPart.name || "Select a specification"}
                </div>
                    ...
                    onChange={(e) => {
                      ...
                      if (group && group.variants.length > 0) {
                        ...
                        setSelectedProduct({
                          ...
                          title: (() => {
                            const p = productInfo.part?.toLowerCase() || "";
                            const partName = p.includes("engine") ? "Engine" : "Transmission";
                            return `${productInfo.year} ${productInfo.make} ${productInfo.model} Used ${partName}`.trim();
                          })(),
                        });
