...
export const createOrderInBackend = async (orderData: any) => {
  try {
    ...
    console.log("Extracted data:", {
      ...
      orderNumber,
      stripePayment: stripePayment ? "present" : "missing",
    });
    ...
    const processedCartItems = cartItems.map((item: any) => {
      const { specification, milesPromised } = parseSubtitle(
        item.subtitle || ""
      );
      ...
      return {
        ...
        milesPromised:
          item.milesPromised !== undefined ? item.milesPromised : milesPromised,
      };
    });
    ...
    const orderPayload = {
      ...
      //   ...stripePayment,
      //   cardData: {
      //     ...stripePayment.cardData,
      //     cardNumber: stripePayment.cardData.cardNumber.slice(-4),
      //   },
      // },
    ...
    console.log(
      "Order payload being sent to backend:",
    ...
    try {
      ...
      const err = new Error(
        `Failed to create order (status ${status}): ${detail}`
      );
